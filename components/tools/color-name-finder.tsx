...
// CSS3 / X11 named colors (sample subset of canonical list — ~140 entries)
...
function distance(a: { r: number; g: number; b: number }, b: { r: number; g: number; b: number }) {
  // Approximated perceptual distance (redmean)
  const rm = (a.r + b.r) / 2;
...
export default function ColorNameFinder() {
  ...
  const matches = useMemo(() => {
    if (!rgb) return [];
    return CSS_COLORS
      .map((c) => ({ ...c, dist: distance(rgb, hexToRgb(c.hex)!) }))
  ...
  return (
    <div className="space-y-4">
      ...
      {rgb && (
        <Panel title="Closest CSS named colors">
          <div className="space-y-2">
            {matches.map((m, i) => (
              <div key={m.name} className="flex items-center gap-3 rounded-md border border-[var(--color-border)] px-3 py-2">
                <div className="h-8 w-8 rounded border border-[var(--color-border)]" style={{ background: m.hex }} />
                <div className="flex-1">
                  <div className="text-sm font-medium">
                    {m.name} {i === 0 && <span className="ml-2 text-[10px] uppercase text-[var(--color-accent)]">closest</span>}
                  </div>
                  <div className="text-xs text-[var(--color-muted)] font-mono">
                    {m.hex} · distance {m.dist.toFixed(1)}
                  </div>
                </div>
                <CopyButton value={m.name} label="name" />
                <CopyButton value={m.hex} label="hex" />
              </div>
            ))}
          </div>
        </Panel>
