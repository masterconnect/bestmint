...
const MATERIAL: Record<string, Family> = {
...
const STEPS = ["50", "100", "200", "300", "400", "500", "600", "700", "800", "900", "A100", "A200", "A400", "A700"];

export default function MaterialPalette() {
  const [copied, setCopied] = useState("");
  const copy = async (v: string) => {
    try { await navigator.clipboard.writeText(v); setCopied(v); setTimeout(() => setCopied(""), 1000); } catch { /* ignore */ }
  };

  return (
    <Panel title="Material Design 2014 palette">
      <p className="text-xs text-[var(--color-muted)] mb-3">Click any swatch to copy its hex.</p>
      <div className="space-y-3 overflow-x-auto">
        {Object.entries(MATERIAL).map(([family, shades]) => (
          <div key={family} className="min-w-fit">
            <div className="text-xs font-medium mb-1 uppercase tracking-wider text-[var(--color-muted)]">{family}</div>
            <div className="grid grid-cols-14 gap-1" style={{ gridTemplateColumns: "repeat(14, minmax(0, 1fr))" }}>
              {STEPS.map((step) => {
                const hex = shades[step];
                if (!hex) return <div key={step} className="h-12 rounded-md border border-dashed border-[var(--color-border)] opacity-30" />;
                return (
                  <button
                    key={step}
                    onClick={() => copy(hex)}
                    className="h-12 rounded-md border border-[var(--color-border)] hover:ring-2 hover:ring-[var(--color-accent)] transition flex flex-col items-center justify-center"
                    style={{ background: hex }}
                    title={`${family} ${step} · ${hex}`}
                  >
                    <span className="text-[9px] font-mono px-1 py-0.5 rounded bg-black/40 text-white">{step}</span>
                  </button>
                );
              })}
            </div>
      ...
      {copied && <div className="mt-3 text-xs text-[var(--color-accent)]">Copied: <span className="font-mono">{copied}</span></div>}
    </Panel>
  );
}
